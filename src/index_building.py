...
class DatastoreBatch():
    ...
    def search(self, queries, k):
        ...
        for i in range(self.batch_size):
            topk = min(self.indices[i].index_size, k) # don't search for more items than we have!
            scores, values = self.indices[i].search(queries[i], topk)
            found_scores.append(scores)
            found_values.append(values)
        return torch.stack(found_scores, dim=0), torch.stack(found_values, dim=0)

    def search_and_reconstruct(self, queries, k, original_order=False):
        found_scores, found_values = [], []
        found_vectors = []
        for i in range(self.batch_size):
            scores, values, vectors = self.indices[i].search_and_reconstruct(queries[i], k, original_order=original_order)
            found_scores.append(scores)
            found_values.append(values)
...
class Datastore():
    ...
    def add_keys(self, keys, num_keys_to_add_at_a_time=1000000, index_is_trained=False):
        ...
        if not self.use_flat_index and index_is_trained:
            ...
            while start < keys.shape[0]:
                ...
                self.index_size += len(to_add)
                start += end
                if (start % 1000000) == 0:
                    self.logger.info(f'Added {start} tokens so far')
        else:
            self.index_size += len(keys)
        # else:
    ...
    def search_and_reconstruct(self, queries, k, original_order):
