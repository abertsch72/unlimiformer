...
class Unlimiformer(Generic[ModelType]):
    def __init__(self, model: ModelType, 
            ...
            index_devices=(0,), datastore_device=0, device=None,
        ...
        self.layer_end = layer_end if layer_end != -1 else None
        ...
        if device is None:
            if self.gpu_datastore and torch.cuda.is_available():
        ...
        self.preserve_ordering = preserve_ordering # return embeddings in the order they were encoded, not in order of max score
    ...
    def window_size(self):
        raise NotImplementedError("should be implemented in your child class!")
    ...
    def reset_memory(self, input_ids, attention_mask):
        ...
        for context_start_ind, context_end_ind, update_start_ind, update_end_ind in window_indices:
            ...
            assert chunk.shape == chunk_attention_mask.shape
            ...
            if self.use_datastore:
                ...
                if not self.reconstruct_embeddings:
                    ...
                    for i, layer_states in enumerate(to_add_embeddings):
                        layer_states = layer_states * to_apply_mask.unsqueeze(-1) #to(self.device if self.gpu_datastore else torch.device('cpu')) 
    ...
    def pre_generate_hook(self, input_ids, keep_datastore: bool = False, force_unlimiformer_prefix: bool = False, prefix = None, suffix=None, **kwargs):
        if 'attention_mask' not in kwargs:
            kwargs['attention_mask'] = torch.ones_like(input_ids)

        # TODO: pass keep_datastore flag in here 
        # TODO: pass a prefix-keeping flag throughout 
        if not keep_datastore:
            self.reset_memory(input_ids, kwargs['attention_mask'])
        else:
            self.generated_input_ids = torch.tensor([], dtype=torch.long, device=input_ids.device) # have to reset generated text at least

        if force_unlimiformer_prefix: # set prefix 
            self.generated_input_ids = prefix.to(input_ids.device)
        ...
        self.prefix = prefix
        self.suffix = suffix
        self.reserved_space = (len(self.prefix[0]) if prefix is not None else 0) + (len(self.suffix[0]) if suffix is not None else 0) 
        if force_unlimiformer_prefix:
            self.prefix = prefix
        else:
            self.prefix = None
        ...
        if 'attention_mask' in kwargs:
            ...
            new_kwargs['attention_mask'] = kwargs['attention_mask'][:, :self.actual_model_window_size - self.reserved_space].to(self.device)
        new_kwargs['use_cache'] = True
        
        if self.is_encoder_decoder:
            input_ids_prefix = input_ids[:, :self.actual_model_window_size - self.reserved_space]
        else:
            input_ids_prefix = input_ids[:, -self.actual_model_window_size + self.reserved_space:]
        
        # add prefix and suffix content 
        if self.prefix is not None:
            input_ids_prefix = torch.cat((self.prefix, input_ids_prefix), dim=1)
            new_kwargs['attention_mask'] = torch.cat((torch.ones(self.prefix.shape).to(self.device), new_kwargs['attention_mask']), dim=1)
        if self.suffix is not None:
            input_ids_prefix = torch.cat((input_ids_prefix, self.suffix), dim=1)
            new_kwargs['attention_mask'] = torch.cat((new_kwargs['attention_mask'], torch.ones(self.suffix.shape).to(self.device)), dim=1)
    ...
    def attention_forward_hook(self, module, input, output):
        ...
        with torch.no_grad():
            ...
            topk = min(prompt_size, window_size - self.reserved_space)
            if not self.is_encoder_decoder:
                topk = min(topk, self.actual_model_window_size - generated_size + 1 - self.reserved_space)
            if self.gpu_index:
                topk = min(topk, 2048)
            ...
            if self.use_datastore:
                ...
                if self.reconstruct_embeddings:
                    ...
                    _, top_search_key_indices, embeddings = self.datastore[datastore_index].search_and_reconstruct(datastore_query, k=topk, original_order=self.preserve_ordering) 
                else:
        ...
        # TODO: put our prefix and suffix into their respective places

        if self.prefix is not None: 
            retrieved_keys = torch.cat([self.cur_layer_key_value_placeholder[0][:,:,:len(self.prefix[0])], retrieved_keys], dim=-2)
            retrieved_values = torch.cat([self.cur_layer_key_value_placeholder[1][:,:,:len(self.prefix[0])], retrieved_values], dim=-2)
            topk += len(self.prefix[0])
...
class UnlimiformerBART(Unlimiformer[BartModel]):
    ...
    def activation_to_capture(self, layer_begin, layer_end): 
        if layer_end == -1:
            layer_end = len(self.model.base_model.decoder.layers) + 1 # fixes the indexing 
        if self.use_datastore:
...
class UnlimiformerLLaMa(Unlimiformer[LlamaModel]):
    ...
    def activation_to_capture(self, layer_begin, layer_end): 
        if layer_end == -1:
            layer_end = len(self.model.base_model.layers) + 1 # fixes the indexing 
    ...
    def preprocess_query(self, query, k_proj_weight):
        ...
        query = query.to(cos.device)
        k_proj_weight = k_proj_weight.to(cos.device)
        cos = cos[:,:,-1]  # [1, 1, dim]
        sin = sin[:,:,-1]  # [1, 1, dim]
    ...
    def post_process_retrieved(self, embeddings, k_proj_layer, v_proj_layer, top_search_key_indices):
        ...
        if v_proj_layer.bias is not None:
            v_bias = v_proj_layer.bias.view(1, self.num_heads, embed_dim // self.num_heads).unsqueeze(-2).unsqueeze(0)
        # new_keys, new_values: (batch, beam, head, encoder_len, attn_dim)
        embeddings = embeddings.to(k_weight.device) # fix wrong-device issue
        ...
        cos = cos.squeeze(1).squeeze(0).to(embeddings.device)  # [seq_len, dim]
        sin = sin.squeeze(1).squeeze(0).to(embeddings.device)  # [seq_len, dim]
